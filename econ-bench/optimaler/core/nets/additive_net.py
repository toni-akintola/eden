...
class AdditiveNet(nn.Module):
    ...
    def create_input_alloc_layer(self):
        alloc_first_layer = nn.Linear(self.num_in, self.num_a_hidden_units).to(
            self.device
        )
    ...
    def create_body_alloc_layer(self):
        for i in range(1, self.num_a_layers - 1):
            alloc_new_layer = nn.Linear(
                self.num_a_hidden_units, self.num_a_hidden_units
            ).to(self.device)
    ...
    def create_head_alloc_layer(self):
        alloc_output_layer = nn.Linear(self.num_a_hidden_units, self.num_a_output).to(
            self.device
        )
    ...
    def create_input_payment_layer(self):
        pay_first_layer = nn.Linear(self.num_in, self.num_p_hidden_units).to(
            self.device
        )
    ...
    def create_body_payment_layer(self):
        for i in range(1, self.num_p_layers - 1):
            pay_new_layer = nn.Linear(
                self.num_p_hidden_units, self.num_p_hidden_units
            ).to(self.device)
    ...
    def create_head_payment_layer(self):
        pay_output_layer = nn.Linear(self.num_p_hidden_units, self.num_agents).to(
            self.device
        )
