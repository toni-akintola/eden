...
class AdditiveNetAttention(AdditiveNet):
    ...
    def create_input_layers(self):
        self.input_layer = Exchangeable(1, self.hid, add_channel_dim=True).to(
            self.device
        )

        if self.config.net.pos_enc:
            if self.config.net.pos_enc_part > 1:
                pos_enc_part_layer = PositionalEncoding(
                    self.config.net.pos_enc_part, self.hid, item_wise=False
                )
                self.input_layer = nn.Sequential(self.input_layer, pos_enc_part_layer)
            if self.config.net.pos_enc_item > 1:
                pos_enc_item_layer = PositionalEncoding(
                    self.config.net.pos_enc_item, self.hid, item_wise=True
                )
    ...
    def create_body_layers(self):
        self.body_layers = nn.ModuleList(
            [
                MHAttentionBody(self.n_heads, self.hid, self.hid_att)
    ...
    def forward(self, x, return_intermediates=False):
        ...
        x = self.activation(
            self.input_layer(x)
        )  # [-1, self.num_agents, self.num_items, self.hid]
