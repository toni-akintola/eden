    ...
    if cfg.regret_type in ["standard", "distillation"]:
        ...
        if cfg.regret_type == "standard":
            ...
            state_dict = torch.load(
                f"target_nets/{cfg.distill.architecture}/setting_{cfg.setting}/seed_{seed}/"
                f"model_200000",
                map_location=device,
            )
        ...
        nets = {
            "RegretNet": AdditiveNet(cfg, device).to(device),
            "EquivariantNet": AdditiveNetExchangeable(cfg, device).to(device),
            "RegretFormer": AdditiveNetAttention(cfg, device).to(device),
        }
        state_dict = torch.load(
            f"target_nets/RegretNet/setting_{cfg.setting}/seed_{seed}/" f"model_200000",
            map_location=device,
        )
        nets["RegretNet"].load_state_dict(state_dict)

        state_dict = torch.load(
            f"target_nets/EquivariantNet/setting_{cfg.setting}/seed_{seed}/"
            f"model_200000",
            map_location=device,
        )
        ...
        state_dict = torch.load(
            f"target_nets/RegretFormer/setting_{cfg.setting}/seed_{seed}/"
            f"model_200000",
            map_location=device,
        )
