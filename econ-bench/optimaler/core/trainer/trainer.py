...
class Trainer(object):
    ...
    def create_constants(self):
        ...
        self.rgt_target_mult = (
            self.config.train.rgt_target_end / self.config.train.rgt_target_start
    ...
    def create_params_to_train(self, train=True, val=True):
        ...
        if val:
            self.adv_var["val"] = torch.zeros(
                self.adv_var_shape["val"], requires_grad=True, device=self.device
            ).float()
    ...
    def create_optimizers(self, train=True, val=True):
        ...
        self.opt2 = dict()
        if train:
            self.opt2["train"] = optim.Adam(
                [self.adv_var["train"]], self.config.train.gd_lr
            )
        if val:
        ...
        self.sc_opt2 = dict()
        if train:
            self.sc_opt2["train"] = optim.lr_scheduler.StepLR(
                self.opt2["train"], 1, self.config.train.gd_lr_step
            )
        if val:
            self.sc_opt2["val"] = optim.lr_scheduler.StepLR(
                self.opt2["val"], 1, self.config.val.gd_lr_step
            )
    ...
    def create_masks(self, train=True, val=True):
        ...
        if train:
            ...
            self.adv_mask["train"][
                np.arange(self.config.num_agents),
                :,
                :,
                np.arange(self.config.num_agents),
                :,
    ...
    def mis_step(self, x):
        ...
        u_mis = -(
            utility_mis.view(self.u_shape[mode]) * self.u_mask[mode].to(self.device)
        ).sum()
    ...
    def compute_regret_grad(self, x, a_true, p_true, x_mis, a_mis, p_mis):
        ...
        utility_true = utility.repeat(
            self.config.num_agents * self.config[mode].num_misreports, 1
        )
        excess_from_utility = F.relu(
            (utility_mis - utility_true).view(self.u_shape[mode])
    ...
    def get_misreports_grad(self, x):
        ...
        x_mis = x.repeat(
            self.config.num_agents * self.config[mode].num_misreports, 1, 1
        )
    ...
    def train_epoch(self, iteration):
        ...
        self.rgt_target = max(
            self.rgt_target * self.rgt_target_mult, self.config.train.rgt_target_end
        )
    ...
    def eval_grad(self, iteration):
        ...
        print(
            "Val revenue: {},   regret_grad: {}".format(
                round(float(val_revenue), 5), round(float(val_regret), 5)
            )
        )
    ...
    def save(self, iteration):
        torch.save(
            self.net.state_dict(), self.writer.log_dir + "/model_{}".format(iteration)
        )
...
class DistillationTrainer(Trainer):
    ...
    def train_epoch(self, iteration):
        ...
        if (iteration % self.config.train.print_iter) == 0:
            ...
            self.writer.add_scalar(
                "Train/revenue_target", target_revenue, iteration / 1000
            )
            self.writer.add_scalar("Train/loss", net_loss, iteration / 1000)
    ...
    def train_op(self, x):
        ...
        alloc_target, final_pay_target, pay_target = self.target_net(
            x, return_intermediates=True
        )
        alloc_target, final_pay_target, pay_target = (
            alloc_target.detach(),
            final_pay_target.detach(),
            pay_target.detach(),
        )
        ...
        p_log, r_p_log, pt_log, r_pt_log = (
            (pay + 1e-5).log(),
            (1 - pay + 1e-5).log(),
            (pay_target + 1e-5).log(),
            (1 - pay_target + 1e-5).log(),
        )
        pay_loss = (
            pay_target * (pt_log - p_log) + (1 - pay_target) * (r_pt_log - r_p_log)
        ).mean()

        a_log, r_a_log, at_log, r_at_log = (
            (alloc + 1e-5).log(),
            (1 - alloc.sum(dim=1) + 1e-5).log(),
            (alloc_target + 1e-5).log(),
            (1 - alloc_target.sum(dim=1) + 1e-5).log(),
        )
        ...
        if self.config.distill.train_misreports:
            ...
            alloc_mis_target, _, pay_mis_target = self.target_net(
                misreports, return_intermediates=True
            )
            alloc_mis_target, pay_mis_target = (
                alloc_mis_target.detach(),
                pay_mis_target.detach(),
            )

            p_log, r_p_log, pt_log, r_pt_log = (
                (pay_mis + 1e-5).log(),
                (1 - pay_mis + 1e-5).log(),
                (pay_mis_target + 1e-5).log(),
                (1 - pay_mis_target + 1e-5).log(),
            )
            pay_loss = (
                pay_mis_target * (pt_log - p_log)
                + (1 - pay_mis_target) * (r_pt_log - r_p_log)
            ).mean()

            a_log, r_a_log, at_log, r_at_log = (
                (alloc_mis + 1e-5).log(),
                (1 - alloc_mis.sum(dim=1) + 1e-5).log(),
                (alloc_mis_target + 1e-5).log(),
                (1 - alloc_mis_target.sum(dim=1) + 1e-5).log(),
            )
            alloc_loss = (alloc_mis_target * (at_log - a_log)).sum(dim=1).mean()
            alloc_loss += (
                (1 - alloc_mis_target.sum(dim=1)) * (r_at_log - r_a_log)
            ).mean()
            final_loss += pay_loss + alloc_loss
    ...
    def eval_grad(self, iteration):
        ...
        print(
            "Val revenue: {}, revenue_target: {}, regret_grad: {}, regret_grad_target: {}".format(
        ...
        self.writer.add_scalar("Validation/regret_grad", val_regret, iteration / 1000)
        self.writer.add_scalar(
            "Validation/revenue_target", val_revenue_t, iteration / 1000
        )
        self.writer.add_scalar(
            "Validation/regret_grad_target", val_regret_t, iteration / 1000
        )
        ...
        if self.config.distill.validate_target_misreports:
            ...
            print(
                "Val target regret_grad: {}, regret_grad_target: {}".format(
                    round(float(val_t_regret), 5), round(float(val_t_regret_t), 5)
                )
            )

            self.writer.add_scalar(
                "Validation_target/regret_grad", val_t_regret, iteration / 1000
            )
            self.writer.add_scalar(
                "Validation_target/regret_grad_target", val_t_regret_t, iteration / 1000
            )
    ...
    def eval_grad(self):
        metrics = {
            "RegretNet": {
                "revenue": 0,
                "regret_RegretNet": 0,
                "regret_EquivariantNet": 0,
                "regret_RegretFormer": 0,
            },
            "EquivariantNet": {
                "revenue": 0,
                "regret_RegretNet": 0,
