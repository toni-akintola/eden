...
class AttentionHead(nn.Module):
    ...
    def forward(self, x):
        ...
        logits = torch.matmul(
            x_agent / self.temperature, x_item.transpose(1, 2)
        )  # [-1, num_agents, num_items]

        logits = torch.cat(
            [logits, -logits.sum(1, keepdim=True)], dim=1
        )  # [-1, num_agents + 1, num_items]
...
class PartAttentionHead(nn.Module):
    ...
    def forward(self, x):
        ...
        x_agent = x_agent.transpose(1, 2).transpose(
            0, 1
        )  # [n_misreports, -1 num_agents, hid]

        x_item = self.encoder_item(x).mean(1)  # [-1, num_items, hid]
        x_item = x_item.transpose(1, 2).unsqueeze(0)  # [1, -1, hid, num_items]

        x = torch.matmul(
            x_agent / self.temperature, x_item
        )  # [n_misreports, -1, num_agents, num_items]
