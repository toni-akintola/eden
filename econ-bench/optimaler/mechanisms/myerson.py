...
def Myerson_item_wise(batch, reserve=None, dist="uniform", **dist_kwargs):
    ...
    allocation = (
        allocation.permute(1, 0)
        .reshape(shape_init[1], shape_init[0], shape_init[2])
        .permute(1, 0, 2)
    )
    prices = (
        prices.permute(1, 0)
...
def Myerson_bundled(batch, reserve=None, dist="uniform", **dist_kwargs):
    ...
    if dist == "uniform":
        ...
        raise Warning(
            "Be sure to change pdf and cdf in dist_kwargs to account for the summation of valuations"
        )

    allocation, prices = Myerson(
        batch.sum(dim=-1), reserve=reserve, dist=dist, **dist_kwargs
    )
