...
def gen_valuations_normal(
    size: object,
    mean_of_mean: object = 10,
    std_of_mean: object = 1,
    mean_of_std: object = 2,
    std_of_std: object = 0.5,
) -> object:
    """
    ...
    std = (
        F.relu(
            torch.distributions.Normal(mean_of_std - 1e-3, std_of_std).sample((size,))
        )
        + 1e-3
    )
    for i in range(size):
...
def gen_auctions_normal(
    ...
    data = gen_valuations_normal(
        size * n_participants * n_items,
        mean_of_mean,
        std_of_mean,
        mean_of_std,
        std_of_std,
...
if __name__ == "__main__":
    auctions = gen_auctions_normal(
        5, n_participants=2, n_items=3, language="unit-demand"
    )
    print(auctions.size())
