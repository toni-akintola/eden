    ...
    for tag in [
        ...
        for scalar in scalars:
            if len(scalar) > len(steps):
                ...
                scalar = [
                    scalar[(i * len(scalar)) // len(steps)] for i in range(len(steps))
                ]
            assert len(steps) == len(
                scalar
            ), f"len(steps) = {len(steps)}, len(scalar) = {len(scalar)}"
...
def to_csv(dpath):
    ...
    multi_idx = pd.MultiIndex.from_tuples(
        [tuple(directory.split("/")[-3:]) for directory in dirs], names=idx_names
    )
