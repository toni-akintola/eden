...
def plot_from_csv():
    for tag in TAGS:
        ...
        for k in RGT_LEVELS.keys():
            if not os.path.exists(PIC_DIR + k):
                os.mkdir(PIC_DIR + k)
        ...
        for setting in df["setting"].unique():
            ...
            for k, v in RGT_LEVELS.items():
                ...
                if tag.endswith("regret_grad.csv"):
                    ...
                    ylim = ylim_rev
                plot_config.process_axes(
                    [ax], log=not tag.endswith("revenue.csv"), ylim=ylim
                )
...
def table_from_csv():
    ...
    # aggregated std
    print(
        df.groupby(["R_max", "setting", "network"])["revenue"]
        .std()
        .dropna()
        .reset_index()
        .groupby("setting")
    ...
    print(
        df.groupby(["R_max", "setting", "network"])["regret"]
        .std()
        .dropna()
        .reset_index()
        .groupby("R_max")
    ...
    df = df.pivot_table(
        index=["R_max", "setting"], columns="network", values=["revenue", "regret"]
    )
    df = df[
        [
            ["revenue", "RegretNet"],
            ["regret", "RegretNet"],
            ["revenue", "EquivariantNet"],
            ["regret", "EquivariantNet"],
            ["revenue", "RegretFormer"],
            ["regret", "RegretFormer"],
        ]
    ]
