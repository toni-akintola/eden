...
def table_from_csv():
    ...
    df["R_max"] = "1e-3"
    df.loc[
        df["network"].isin(["standard_tight", "exchangeable_tight", "attention_tight"]),
        "R_max",
    ] = "1e-4"
    ...
    df["regret_train_ratio"] = (
        df["regret_train"] / df["revenue_train"] / df["R_max"].astype(float)
    )
    df["regret_val_ratio"] = (
        df["regret"]
        / df["revenue"]
        / df["R_max"].astype(float)
        * df["setting"].apply(lambda x: int(x[0]))
    )

    df = df.pivot_table(
        index=["R_max", "setting"],
        columns="network",
        values=["regret_train_ratio", "regret_val_ratio"],
    )
    df = df[
        [
            ["regret_train_ratio", "RegretNet"],
            ["regret_val_ratio", "RegretNet"],
            ["regret_train_ratio", "EquivariantNet"],
            ["regret_val_ratio", "EquivariantNet"],
            ["regret_train_ratio", "RegretFormer"],
            ["regret_val_ratio", "RegretFormer"],
        ]
    ]
...
if __name__ == "__main__":
    ...
    RGT_LEVELS = {
        "normal/": ["standard", "exchangeable", "attention"],
        "tight/": ["standard_tight", "exchangeable_tight", "attention_tight"],
    }
    COLORS = {
        "standard": Neon.CYAN.norm,
        "exchangeable": Neon.GREEN.norm,
        "attention": Neon.RED.norm,
    }
    LINESTYLES = {"standard": "--", "exchangeable": "-.", "attention": "-"}
    NAMES = {
        "standard": "RegretNet",
        "exchangeable": "EquivariantNet",
        "attention": "RegretFormer",
    }

    TAGS = {
        "revenue": "Validation_revenue.csv",
        "regret": "Validation_regret_grad.csv",
        "revenue_train": "Train_revenue.csv",
        "regret_train": "Train_regret.csv",
    }
