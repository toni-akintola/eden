...
def main(
    ...
    if simulation_duration is None:
        simulation_duration = click.prompt(
            "Enter simulation duration per evaluation",
            type=float,
            default=100.0,
        )
    ...
    if explore_model is None:
        explore_model = click.prompt(
            "   1. EXPLORE stage model (analyzes patterns)",
    ...
    if refine_model is None:
        refine_model = click.prompt(
            "   2. REFINE stage model (optimizes strategy)",
            type=click.Choice(available_models, case_sensitive=True),
    ...
    if act_model is None:
        act_model = click.prompt(
            "   3. ACT stage model (makes final decision)",
            type=click.Choice(available_models, case_sensitive=True),
    ...
    click.echo(f"Simulation duration: {simulation_duration}")
