    ...
    for iteration in range(1, num_iterations + 1):
        ...
        try:
            ...
            if verbose:
                ...
                click.echo(f"  Expected Queue Length E[k]: {sim_results.expected_queue_length_E_k:.2f}")
                click.echo(f"  Expected Service Flow E[mu_k]: {sim_results.expected_service_flow_E_mu_k:.2f}")
                click.echo(f"  Agents Served: {sim_results.num_served}")
                click.echo(f"  V_surplus: {model.V_surplus:.2f}, C_cost: {model.C_waiting_cost:.2f}")
                click.echo(f"  R_profit: {model.R_provider_profit:.2f}, alpha: {model.alpha_weight:.2f}")
                click.echo()
...
def _generate_observations(config: dict, results: SimulationResults, welfare_score: float) -> str:
    ...
    # Add insights
    if E_k > 10:
        observations.append("High queue length - consider increasing service rate or adding exit rules")
    elif E_k < 1:
...
def main(iterations, sim_time, models, output, quiet, task):
    ...
    if models:
        if len(models) != 3:
            click.echo("Error: Must provide exactly 3 model names (explore, refine, act)", err=True)
            sys.exit(1)
